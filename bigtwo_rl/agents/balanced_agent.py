...
class BalancedRandomAgent(BaseAgent):
    ...
    def get_action_with_moves(
        self, legal_moves: List[List[int]], action_mask=None, observation=None
    ) -> int:
        ...
        if action_mask is not None:
            legal_actions = np.where(action_mask)[0]
            valid_moves = [
                legal_moves[i] for i in legal_actions if i < len(legal_moves)
            ]
        else:
    ...
    def _categorize_moves(
        self, moves: List[List[int]], action_indices: List[int]
    ) -> Dict[str, List[int]]:
    ...
    def _select_move_type(self, available_categories: Dict[str, List[int]]) -> str:
        ...
        weights = [
            self.move_type_weights.get(move_type, 0.1) for move_type in available_types
        ]
...
class MoveTypeBalancedWrapper:
    ...
    def __init__(
        ...
        move_type_weights: Optional[Dict[str, float]] = None,
    ):
