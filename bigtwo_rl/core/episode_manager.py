...
class EpisodeManager:
    ...
    def __init__(self, games_per_episode: int = 10, num_players: int = 4, controlled_player: int = 0):
    ...
    def handle_game_end(self, all_cards_left: List[int]) -> Tuple[int, bool]:
        ...
        position = next(
            i for i, (player_idx, _) in enumerate(sorted_positions) 
        ...
        opponent_cards = sum(
            cards for i, cards in enumerate(all_cards_left) 
    ...
    def get_episode_metrics(self) -> Dict[str, float]:
        ...
        final_position_avg = (
            ...
            if self._final_positions else 0
        )
        
        # Opponent analysis
        avg_opponent_cards_per_game = (
            self._total_opponent_cards / (self.games_played * (self.num_players - 1))
        )
