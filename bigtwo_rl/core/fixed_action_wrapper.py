"""Big Two RL Environment wrapper with fixed 1,365-action space.

This wrapper provides the same interface as BigTwoRLWrapper but uses a fixed action space
where the network always outputs exactly 1,365 logits, with proper action masking to
enforce game rules.
"""
...
class FixedActionBigTwoWrapper(gym.Env):
    ...
    This wrapper maintains compatibility with the existing BigTwoRLWrapper interface
    while using a fixed action space that enables better policy learning through
    consistent action representations across all game states.
    
    Key differences from BigTwoRLWrapper:
    - Fixed action space of exactly 1,365 actions
    - Proper action masking using game rules
    - Direct action ID to move translation
    - Backward compatible with existing training/evaluation code
    """
    ...
    def __init__(
        ...
        """Initialize Fixed Action Big Two RL wrapper.
        ...
        # Use standard observation if None is provided
        if observation_config is None:
            from .observation_builder import standard_observation
        ...
        self._current_obs: Optional[np.ndarray] = None  # Track latest observation
    ...
    def _execute_player_action(
        self, player: int, legacy_action: int, current_obs: np.ndarray, fixed_action_id: int
    ) -> float:
