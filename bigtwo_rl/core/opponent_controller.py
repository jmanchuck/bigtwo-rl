...
class OpponentController:
    ...
    def __init__(
        self, num_players: int = 4, controlled_player: int = 0, opponent_provider=None
    ):
    ...
    def should_autoplay_current_player(self, current_player: int) -> bool:
        ...
        return self.has_opponents() and current_player != self.controlled_player
    ...
    def select_opponent_action(
        ...
        agent = (
            self._episode_opponents.get(player_idx) if self._episode_opponents else None
        )
        if agent is None:
