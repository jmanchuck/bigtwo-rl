...
class MultiPlayerExperienceBuffer:
    ...
    def _calculate_retroactive_reward(
        ...
        length_factor = max(
            0.5, 1.0 - (game_length - 20) / 100
        )  # Normalize around 20 moves
    ...
    def _apply_episode_bonuses(self) -> None:
        ...
        for player_idx in range(4):
            ...
            for exp in player_experiences[
                -10:
            ]:  # Last 10 experiences get episode bonus
    ...
    def _experiences_to_arrays(
        ...
        if experiences[0]["action_log_prob"] is not None:
            batch["action_log_probs"] = np.array(
                [exp["action_log_prob"] for exp in experiences]
            )
    ...
    def clear(self) -> None:
        ...
        self.player_experiences = {
            i: deque(maxlen=self.max_buffer_size) for i in range(4)
        }
        self.current_episode_experiences = {i: [] for i in range(4)}
        self.current_game_experiences = {i: [] for i in range(4)}
        self.episode_count = 0
