...
class SelfPlayPPOCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        for env_idx in range(self.training_env.num_envs):
            ...
            if hasattr(self.locals, 'infos') and len(self.locals['infos']) > env_idx:
                ...
                if (info.get('episode_complete', False) and 
                    'multi_player_experiences' in info):
                    
                    self._process_multi_player_experiences(
                        info['multi_player_experiences'], 
                        env_idx
                    )
    ...
    def _on_rollout_start(self) -> None:
        ...
        if hasattr(self, '_pending_experiences') and self._pending_experiences:
            if self.verbose >= 1:
                self.logger.record(
                    "self_play/pending_experiences", 
                    len(self._pending_experiences)
                )
    ...
    def _log_multi_player_stats(self) -> None:
        ...
        if self.verbose >= 1:
            ...
            self.logger.record("self_play/total_multi_player_experiences", self.total_multi_player_experiences)
    ...
    def _on_training_end(self) -> None:
        ...
        if self.verbose >= 1:
            ...
            print(f"Total multi-player experiences collected: {self.total_multi_player_experiences}")
            print("Experiences per player:")
            for i in range(4):
                print(f"  Player {i}: {self.experiences_per_player[i]}")
            
            if self.total_episodes_processed > 0:
                avg_exp = self.total_multi_player_experiences / self.total_episodes_processed
                print(f"Average experiences per episode: {avg_exp:.1f}")
                print(f"Training data multiplier: {avg_exp/10:.1f}x (compared to single-player)")
            print("="*50 + "\n")
...
class SimpleSelfPlayCallback(BaseCallback):
    ...
    def _on_training_start(self) -> None:
        ...
        if self.verbose >= 1:
            print(f"Self-play callback: Training start - env type: {type(self.training_env)}")
            
        # Handle different VecEnv types
        if hasattr(self.training_env, 'envs'):
            # DummyVecEnv case - direct access to environments
            if self.verbose >= 1:
                print(f"Self-play callback: Found {len(self.training_env.envs)} environments in {type(self.training_env)}")
                
            for i, env in enumerate(self.training_env.envs):
                target_env = self._get_base_env(env)
                if self.verbose >= 1:
                    print(f"Self-play callback: Env {i} - target type: {type(target_env)}, has method: {hasattr(target_env, 'set_model_reference')}")
            ...
            try:
                ...
                self.model_injected = True
                if self.verbose >= 1:
                    print(f"Self-play callback: Model reference injected via env_method")
            except Exception as e:
                if self.verbose >= 1:
                    print(f"Self-play callback: env_method injection failed: {e}")
                    print("Model sharing not supported with SubprocVecEnv - consider using DummyVecEnv")
        else:
            # Single env case
            target_env = self._get_base_env(self.training_env)
            if self.verbose >= 1:
                print(f"Self-play callback: Single env - target type: {type(target_env)}, has method: {hasattr(target_env, 'set_model_reference')}")
        ...
        if self.verbose >= 1:
            print(f"Self-play callback: Model reference injected into environments: {self.model_injected}")
