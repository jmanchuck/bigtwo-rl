...
class StrategicReward(BaseReward):
    ...
    def move_bonus(self, move_cards: list[int]) -> float:
        ...
        if len(move_cards) == 2:
            return 0.2
        if len(move_cards) == 3 or len(move_cards) == 5:
            return 0.5
        return 0.0
    ...
    def episode_bonus(self, games_won: int, total_games: int, avg_cards_left: float) -> float:
        ...
        if win_rate >= 0.4 and avg_cards_left <= 5.0:
            return 0.6  # Good strategic play
        if avg_cards_left <= 3.0:  # Good card management even if fewer wins
            return 0.4
        if win_rate >= 0.5:  # Decent win rate
            return 0.2
