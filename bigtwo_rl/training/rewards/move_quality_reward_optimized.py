...
class MoveQualityRewardOptimized(BaseReward):
    ...
    def move_bonus(self, move_cards: List[int], game_context: Optional[Dict] = None) -> float:
        ...
        weights = np.array([self.card_efficiency_weight, self.hand_type_weight, 
                           self.timing_weight, self.preservation_weight])
    ...
    def _timing_score_fast(self, move_strength: int, opponent_card_counts: List[int], 
    ...
    def game_reward(self, winner_player: int, player_idx: int, cards_left: int,
                   all_cards_left: Optional[List[int]] = None) -> float:
