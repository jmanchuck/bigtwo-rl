...
def play_single_game(agents: List[BaseAgent], env: BigTwoRLWrapper):
    ...
    while not done:
        ...
        if current_player < len(agents):
            ...
            # Ensure observation is right shape for PPO agents
            if hasattr(agent, 'model'):  # PPO agent
                # Make sure observation is the right shape
                obs_array = np.array(obs, dtype=np.float32)
                if obs_array.shape != (109,):
                    print(f"Warning: Observation shape mismatch: {obs_array.shape} vs (109,)")
                    # Pad or truncate to correct size
                    if obs_array.shape[0] < 109:
                        obs_array = np.pad(obs_array, (0, 109 - obs_array.shape[0]))
                    elif obs_array.shape[0] > 109:
                        obs_array = obs_array[:109]
                action = agent.get_action(obs_array, action_mask)
            else:
...
def _create_tournament_summary(agents: List[BaseAgent], matchup_results: List[Dict]) -> str:
    ...
    for result in matchup_results:
        summary.append(f"{result['agent1']} vs {result['agent2']}: "
