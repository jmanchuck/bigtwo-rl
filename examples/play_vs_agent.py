...
class HumanVsAgentGame:
    ...
    def display_game_state(self):
        ...
        print("\n" + "="*50)
        current_player_name = "YOU" if self.env.current_player == self.human_player else f"AGENT {self.env.current_player}"
        print(f"Current player: {current_player_name}")
        
        if self.env.last_play:
            from card_utils import hand_to_strings
        ...
        print(f"Hand sizes: YOU={hand_sizes[0]}, AGENT1={hand_sizes[1]}, AGENT2={hand_sizes[2]}, AGENT3={hand_sizes[3]}")
        from card_utils import format_hand
    ...
    def play_game(self):
        ...
        print("Suits: D < C < H < S")
        print("Play types: Singles, Pairs, Trips, 5-card hands (Straight/Flush/Full House/Four of a Kind/Straight Flush)")
        ...
        hand_sizes = [len(hand) for hand in self.env.hands]
        print(f"Final hand sizes - YOU: {hand_sizes[0]}, AGENT1: {hand_sizes[1]}, AGENT2: {hand_sizes[2]}, AGENT3: {hand_sizes[3]}")
        print("="*50)
...
def main():
    ...
    try:
        ...
        print("Run training first: uv run train.py")
    except KeyboardInterrupt:
        print("\nGoodbye!")
